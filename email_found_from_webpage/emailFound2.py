...
def process_keywords(input_file):
    try:
        ...
        for count, (_, row) in enumerate(df.iterrows()):
            try:
                ...
                if count % 50 == 0 and count > 0:
                    pd.DataFrame(results).to_excel(f"output-emails2-{count}.xlsx", index=False, engine="openpyxl")
        ...
        # Final write
        pd.DataFrame(results).to_excel("output-emails2-final.xlsx", index=False, engine="openpyxl")
